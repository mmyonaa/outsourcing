...
| 버전 | 날짜 | 주요 내용 |
|------|------|-----------|
| **v1.5.0** | 2026-08-19 | 전면 코드 리뷰 스프린트 — 데이터 파괴·SEO 회귀 등 버그 60여 건 수정, 업로드 화이트리스트·조회수 전용 API 등 보안 보강, 이전/다음 글·서브탭·라이트박스 UI, 복붙 뷰 20종 → 공용 페이지 통합(-4,000줄) |
| **v1.4.1** | 2026-07-21 | 검색 색인 수정 — vite-ssg 프리렌더 복구(라우터 가드 `next()` 누락으로 스킵되던 문제) + 페이지별 SEO(title·canonical·JSON-LD) 프리렌더 주입 |
| **v1.4.0** | 2026-07-08 | UI/UX 개선 — 로딩/에러 상태, 스크롤 리빌·연혁 타임라인·푸터 개편, 접근성, HTTPS 전환, API 문서(Swagger) |
