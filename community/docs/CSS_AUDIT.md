...
## 진행 현황 (2026-06-25 업데이트)

| 우선순위 | 상태 | 커밋 |
|---------|------|------|
| **P0** 모바일 깨짐 (3건) | ✅ 완료 | `124837e` |
| **P1** 카드/태그 일관성 (4·5·6) | ✅ 완료 (7=기존충족·8=보류) | `c1e2986` 외 |
| **P2** 디자인 토큰 신설/적용 | ✅ 1차 완료 (gray 전면, radius/shadow/z/container 정의) | `e74ab70` |
| **P3** 정리 | ✅ 부분 완료 (미사용변수·터치타깃) | `cfe2807` |

**남은 점진 이전 대상(무리한 일괄 변경 지양):**
- `.vue` scoped 스타일은 plain CSS라 토큰 미적용 → lang="scss" 전환 시 적용 가능
- z-index 토큰은 정의만, 기존 stacking 재배선은 미적용
- radius/shadow 전면 토큰화(카드 외), `!important` 정리, hover/transition 믹스인화
