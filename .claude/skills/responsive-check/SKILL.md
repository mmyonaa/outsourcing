...
argument-hint: "[optional: page path, e.g. /notice]"
---
...
1. **서버 기동** — dev 서버가 안 떠 있으면 `community/frontend`에서 `npm run dev` (백그라운드).
   URL은 `http://localhost:4000/` (vite `--port 4000` 고정).
   포트는 4000 하나만 쓰고, 넓은 `pkill` 금지 — 이미 떠 있으면 재사용한다.
...
2. **각 기기에서 순회** — Playwright **headless**로 위 3종 뷰포트 각각:
   ...
   - **E2E식 인터랙션**: nav/햄버거 메뉴 열고 닫기, 주요 링크 클릭해 실제 진입,
     카드·그리드·토글·검색바 등 상호작용 요소를 눌러 상태 전환까지 확인.
     한 페이지만 보지 말고 홈 → 게시판 목록(공지사항·보도자료) → 게시글 상세까지
     실제 이동 경로를 따라간다.
...
4. **수정 → 재검증** — CSS(SCSS)를 고치고 **같은 기기 뷰포트로 다시 열어** 통과 확인.
...
- 스타일 변수는 `src/assets/css/variables.scss`($colorPrimary 등)를 재사용한다.
